...
public class PersistenceTests extends MongoDbTestBase{
    ...
    void duplicateKeyError() {
        ...
        StepVerifier.create(repository.save(pe)).expectError(DuplicateKeyException.class).verify();
    }

    @Test
    void optimisticLockError() {
        ProductEntity pe1 = repository.findById(savedEntity.getId()).block();
        ProductEntity pe2 = repository.findById(savedEntity.getId()).block();

        pe1.setName("n1");
        repository.save(pe1).block();
        //  Update the entity using the second entity object.
        // This should fail since the second entity now holds a old version number, i.e. a Optimistic Lock Error
        StepVerifier.create(repository.save(pe2)).expectError(OptimisticLockingFailureException.class).verify();

        StepVerifier.create(repository.findById(pe1.getId()))
                .expectNextMatches(updatedEntity -> updatedEntity.getName().equals("n1") && updatedEntity.getVersion() == 1)
                .verifyComplete();
    }
